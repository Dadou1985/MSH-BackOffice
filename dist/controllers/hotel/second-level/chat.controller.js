...
export const updateChatRoomMessage = async (req, res) => {
    ...
    try {
        ...
        if (!chat)
            return res.status(404).json({ message: 'Chat not found' });
        const message = chat.chatRoom.find((i) => i._id.toString() === messageId);
        if (!message)
...
export const deleteChatRoomMessage = async (req, res) => {
    const { hotelId, chatId, messageId } = req.params;
    try {
        const hotel = await Hotel.findById(hotelId);
        if (!hotel)
