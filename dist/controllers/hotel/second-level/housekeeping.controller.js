...
const getCategoryArray = async (hotel, category) => {
...
// CREATE
export const addHousekeepingItem = async (req, res) => {
...
// READ
export const getHousekeepingItems = async (req, res) => {
    const { hotelId, category } = req.params;
    ...
    try {
        ...
        if (!hotel)
            return res.status(404).json({ message: 'Hotel not found' });
        if (category) {
            const items = await getCategoryArray(hotel, category);
...
// UPDATE
export const updateHousekeepingItem = async (req, res) => {
    ...
    try {
        ...
        if (!hotel)
            return res.status(404).json({ message: 'Hotel not found' });
        const categoryArray = await getCategoryArray(hotel, category);
        const item = categoryArray.find((i) => i._id.toString() === itemId);
        if (!item)
...
// DELETE
export const deleteHousekeepingItem = async (req, res) => {
